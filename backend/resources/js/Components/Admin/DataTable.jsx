            ...
            {loading ? (
              ...
              <tr>
                <td
                  ...
                  className="px-6 py-16 text-center text-dark-green/40 font-bold italic text-sm"
                >
                  {emptyMessage}
                </td>
              </tr>
      ...
      {pagination && (
        <div className="p-6 border-t border-forest-green/10 flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="text-[10px] font-black uppercase tracking-widest text-dark-green/40 order-2 sm:order-1">
            ...
            sur <span className="text-forest-green">{pagination.total}</span>
          </span>
            ...
            {pagination.links?.map((link, key) =>
              link.url ? (
                <Link
                  key={key}
                  href={link.url}
                  className={`px-4 py-2 rounded-xl text-xs transition-all font-bold min-w-[2.5rem] text-center ${
                    link.active
                ...
                <span
                  key={key}
                  className="px-4 py-2 rounded-xl text-xs text-dark-green/20 font-bold min-w-[2.5rem] text-center border border-transparent"
                >
                  {link.label.includes('Previous')
                    ? '←'
                    : link.label.includes('Next')
