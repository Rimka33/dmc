          ...
          <motion.div variants={itemVariants} className="flex flex-wrap gap-4">
            {stats.pending_orders > 0 && (
              <Link
                ...
                className="px-4 py-2 bg-amber-500/10 border border-amber-500/20 text-amber-700 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-amber-500/20 transition-all"
              >
            ...
            {stats.new_messages > 0 && (
              <Link
                href="/admin/messages"
                className="px-4 py-2 bg-blue-500/10 border border-blue-500/20 text-blue-700 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-blue-500/20 transition-all"
              >
            ...
            {stats.pending_reviews > 0 && (
              <Link
                href="/admin/reviews"
                className="px-4 py-2 bg-purple-500/10 border border-purple-500/20 text-purple-700 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-purple-500/20 transition-all"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div variants={itemVariants} className="lg:col-span-2">
            <ChartCard
              ...
              subtitle="Ventes et performances financières hebdomadaires"
            >
                ...
                {[
                  {
                    ...
                    color: 'from-forest-green to-neon-green',
                  },
                  {
                    icon: Upload,
                    label: 'Import Données',
                    href: '/admin/products',
                    color: 'from-forest-green to-emerald-600',
                  },
                  {
                    icon: Download,
                  ...
                  },
                  {
                    ...
                    label: 'Factures',
                    href: '/admin/orders',
                    color: 'from-neon-green to-forest-green',
                  },
                ].map((action) => (
                  <Link
                    key={action.label}
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-forest-green/5">
                    {(stats.recent_orders || []).map((order) => (
                      <tr
                        key={order.id}
                        className="group hover:bg-forest-green/5 transition-colors"
                      >
                        <td className="py-4">
                          <p className="text-sm font-black text-dark-green">
                            #{order.order_number}
                          </p>
                          <p className="text-[10px] text-dark-green/40 font-bold">
                        ...
                        <td className="py-4">
                          <p className="text-sm font-black text-forest-green">
              ...
              <Section title="Top Produits" icon={Zap}>
                <div className="space-y-4">
                  {(stats.top_products || []).map((product, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-4 p-3 bg-forest-green/5 rounded-xl border border-forest-green/10 group hover:border-forest-green/30 transition-all"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-dark-green truncate uppercase tracking-tighter">
                          {product.name}
                        </p>
                        <p className="text-[10px] text-dark-green/40 font-black">
                          {product.total_sold} VENTES
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-black text-dark-green">
                ...
                <Section title="Alertes Stock" icon={AlertTriangle} className="border-red-500/10">
                  <div className="space-y-3">
                    {stats.low_stock_products.slice(0, 3).map((product) => (
                      <div
                        key={product.id}
                        className="p-3 bg-red-500/5 rounded-xl border border-red-500/10 flex items-center justify-between"
                      >
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-dark-green truncate">
                            {product.name}
                          </p>
                        ...
                        <Link
                          ...
                          className="p-2 bg-white rounded-lg shadow-sm text-red-500 hover:bg-red-50"
                        >
          ...
          <Section
            ...
            actions={
              <Link
                ...
                className="text-xs font-black uppercase tracking-widest text-forest-green hover:underline"
              >
                Paramètres avancés →
              </Link>
            }
              ...
              {[
                ...
                return (
                  <Link
                    ...
                    href={
                      collection
                        ...
                        : `/admin/collections/create?type=${section.type}`
                    }
                  ...
                  >
                    ...
                    <h3 className="text-sm font-black text-dark-green uppercase tracking-tighter mb-1">
                      {section.label}
                    </h3>
