          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { label: 'Numéro de commande', value: order.order_number },
              {
                label: 'Date',
                value:
                  order.created_at_formatted || new Date(order.created_at).toLocaleDateString(),
              },
              ...
              >
                <span className="text-[10px] font-black text-gray-500 uppercase tracking-wider block mb-2">
                  {item.label}
                </span>
                <span
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="divide-y divide-gray-100">
                  {order.items &&
                    order.items.map((item, index) => (
                      <div
                        key={`order-item-${item.id || index}-${item.product_id || ''}`}
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[10px] text-gray-400">
                            {item.price_formatted || `${item.price.toLocaleString()} F CFA`} / unité
                          </p>
                        </div>
                      </div>
                    ))}
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <h3 className="text-sm font-black text-gray-900 uppercase mb-4 pb-3 border-b">
                ...
                <ul className="space-y-3 text-sm">
                  {[
                    ...
                    <li key={`step-${idx}`} className="flex gap-3">
                      <span className="w-6 h-6 bg-white/20 rounded-full flex items-center justify-center text-xs flex-shrink-0 font-bold">
                        {idx + 1}
                      </span>
