...
function ProductCard({ product, viewMode = 'grid' }) {
  ...
  if (viewMode === 'list') {
    return (
      <div className="bg-white rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group border border-gray-100 flex flex-col md:flex-row gap-6 p-4 md:p-5 relative items-center md:items-stretch">
        <Link to={`/produit/${product.id}`} className="flex-shrink-0 w-full md:w-auto">
          <div className="relative w-full md:w-48 aspect-square overflow-hidden bg-white rounded-xl flex-shrink-0 mx-auto">
            <ShimmerImage
            ...
            />
            {product.is_on_sale && (
              <span className="absolute top-2 left-2 px-1.5 py-0.5 bg-red-600 text-white text-[9px] font-black uppercase rounded shadow-lg">
                SOLDE
              </span>
            )}
          </div>
        </Link>
        <div className="flex flex-col flex-grow text-left w-full py-2">
          <div className="mb-1">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
              {product.category_name}
            </span>
          </div>
          <Link to={`/produit/${product.id}`} className="group block mb-1">
            <h3 className="text-base font-bold text-gray-900 group-hover:text-forest-green transition-colors leading-tight line-clamp-2">
              {product.name}
            </h3>
          </Link>
          ...
          <div className="flex items-center gap-1 mb-3">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`w-3.5 h-3.5 ${i < Math.floor(product.rating || 5) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200 fill-gray-200'}`}
              />
            ))}
          </div>

          <div className="text-gray-500 text-xs font-medium line-clamp-2 mb-4 max-w-2xl leading-relaxed">
            {product.description ||
          ...
          <div className="mt-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                ...
                className="px-5 py-2.5 bg-[#058031] text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-forest-green transition-all"
              >
                VOIR DETAILS
              </Link>
            </div>
  ...
  return (
    <div className="group relative bg-white flex flex-col h-full transition-all border border-gray-100 rounded-xl overflow-hidden hover:shadow-xl">
      <Link
        to={`/produit/${product.id}`}
        className="block relative aspect-square overflow-hidden bg-white mb-1"
      >
        <ShimmerImage
          src={product.primary_image || '/images/products/default.png'}
          alt={product.name}
      ...
      <div className="flex flex-col flex-grow text-left px-3 pb-3">
        ...
        <Link
          to={`/produit/${product.id}`}
          className="block group-hover:text-forest-green transition-colors"
        >
    ...
    <MainLayout>
      <Head>
        ...
        <meta
          ...
          content={
            ...
            `Découvrez notre sélection de produits dans la catégorie ${category?.name} chez DMC.`
          }
        />
      </Head>
      ...
      <section className="relative bg-[#021008] h-[340px] md:h-[400px] flex items-center justify-center z-10">
        ...
        <div className="absolute bottom-0 translate-y-1/2 left-1/2 -translate-x-1/2 w-48 h-48 bg-white rounded-3xl shadow-2xl flex flex-col items-center justify-center p-4 border border-gray-50 group">
          ...
          <h2 className="text-xs font-black text-gray-900 uppercase truncate w-full text-center px-2">
            {category?.name}
          </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="space-y-6">
                  <div className="relative h-2 bg-gray-100 rounded-full">
                    ...
                    <input
                      ...
                      className="absolute w-full top-0 appearance-none bg-transparent pointer-events-none z-30 [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-forest-green [&::-webkit-slider-thumb]:appearance-none"
                    />
                    <input
                      type="range"
                      min={MIN_PRICE}
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-[#058031] text-white text-[10px] font-black uppercase rounded-lg hover:bg-forest-green"
                  >
                    Appliquer
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  {categories.map((cat) => (
                    <Link
                      ...
                      className={`flex items-center gap-2 text-[11px] font-bold uppercase transition-colors ${cat.id === category?.id ? 'text-forest-green' : 'text-gray-500 hover:text-gray-900'}`}
                    >
                      <div
                        className={`w-3 h-3 rounded-sm border ${cat.id === category?.id ? 'bg-forest-green border-forest-green' : 'border-gray-200'}`}
                      ></div>
                      {cat.name}
                    </Link>
                  ))}
            ...
            <div className="lg:col-span-9">
              <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100 uppercase text-[10px] font-black text-gray-500">
                <div>
                  Affichage de {products.length} sur {meta.total} produits
                </div>
                <div className="flex gap-4">
                  <button
                    onClick={() => setViewMode('grid')}
                    className={viewMode === 'grid' ? 'text-forest-green' : 'text-gray-300'}
                  >
                    <Grid className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => setViewMode('list')}
                    className={viewMode === 'list' ? 'text-forest-green' : 'text-gray-300'}
                  >
                      ...
                      <div className="flex items-center gap-2">
                        {[...Array(meta.last_page)].map((_, i) => {
                          ...
                          if (p === page - 2 || p === page + 2)
                            return (
                              <span key={p} className="text-gray-300 font-bold px-1">
                                ...
                              </span>
                            );
                          return null;
