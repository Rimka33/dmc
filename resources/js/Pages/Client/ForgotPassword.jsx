    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                setStatus({ type: 'success', message: 'Un lien de réinitialisation a été envoyé à votre adresse e-mail.' });
                setEmail('');
            }
        } catch (error) {
            setStatus({
                type: 'error',
                message: error.response?.data?.message || 'Une erreur est survenue. Veuillez vérifier votre adresse e-mail.'
            });
        } finally {
            setProcessing(false);
        }
            ...
            <section className="bg-light-gray-bg py-4 border-b">
                <div className="container mx-auto px-4">
                    <div className="flex items-center gap-2 text-sm justify-center">
                        ...
                        <Link to="/connexion" className="text-forest-green hover:underline">Connexion</Link>
                        <ChevronRight className="w-3 h-3 text-gray-400" />
                ...
                <div className="container mx-auto px-4">
                    <div className="max-w-md mx-auto">
                        <div className="text-center mb-10">
                            ...
                            <h1 className="text-2xl font-black text-gray-900 uppercase italic mb-3">Mot de passe oublié ?</h1>
                        ...
                        {status.message && (
                            <div className={`p-4 rounded-lg mb-6 text-sm font-bold ${status.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'
                                }`}>
                                {status.message}
                            </div>
                        )}
