...
import { ArrowLeft, User, Mail, Phone, MapPin, ShoppingCart, Calendar, DollarSign, Package } from 'lucide-react';
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-2xl font-black text-gray-900">{formatCurrency(stats.total_spent)}</p>
                        </div>
                    </div>
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex items-center gap-4">
                        ...
                        <Section title="Informations Personnelles" icon={User}>
                            <div className="space-y-4">
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">Inscrit le</p>
                                        <p className="text-gray-900 font-bold">{new Date(customer.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                                    </div>
                                </div>
                            </div>
                        </Section>
                                ...
                                {customer.addresses && customer.addresses.length > 0 ? (
                                    customer.addresses.map((address, idx) => (
                                        <div key={idx} className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                                            ...
                                            <p className="text-gray-600 text-sm">{address.city}, {address.postal_code}</p>
                                        </div>
                                    ))
                                ) : (
                                    <p className="text-gray-400 italic text-sm text-center py-4">Aucune adresse enregistrée</p>
                                )}
                            </div>
                        </Section>
                    </div>
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="border-b border-gray-100">
                                            <th className="py-4 px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest">N° Commande</th>
                                            <th className="py-4 px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest">Date</th>
                                            <th className="py-4 px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Articles</th>
                                            <th className="py-4 px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest">Statut</th>
                                            ...
                                            orders.data.map((order) => (
                                                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                                                    <td className="py-4 px-2 font-bold text-gray-900">
                                                        <Link href={`/admin/orders/${order.id}`} className="hover:text-forest-green underline">
                                                    ...
                                                    <td className="py-4 px-2">
                                                        <span className={`px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${order.status === 'delivered' ? 'bg-green-50 text-green-600' :
