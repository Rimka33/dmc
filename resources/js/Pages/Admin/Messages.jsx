    ...
    const columns = [
        {
            ...
            label: 'Sujet',
            width: '25%',
            render: (value) => <p className="font-bold text-gray-900 truncate">{value || 'Sans sujet'}</p>
        },
        {
            key: 'name',
            label: 'Nom',
        ...
        },
        {
            ...
            label: 'Message',
            width: '20%',
            render: (value) => <p className="text-gray-600 text-sm truncate">{value ? (value.length > 50 ? value.substring(0, 50) + '...' : value) : '-'}</p>
        },
        {
            key: 'status',
            label: 'Statut',
            ...
            render: (value) => (
                <span className={`px-2 py-1 rounded-full text-xs font-bold ${
                    ...
                    value === 'read' ? 'bg-gray-50 text-gray-600' :
                    value === 'replied' ? 'bg-green-50 text-green-600' :
                    'bg-yellow-50 text-yellow-600'
                }`}>
                    {value === 'new' ? '🆕 Nouveau' : 
                     value === 'read' ? '📖 Lu' : 
                     value === 'replied' ? '✓ Répondu' : 
                     '📦 Archivé'}
                </span>
            )
        },
            ...
            render: (value, row) => (
                <div className="flex items-center gap-2 justify-end">
                    ...
                    {row.status !== 'archived' && (
                        <button
                            onClick={() => {
                                router.post(`/admin/messages/${value}/archive`, {}, {
                                    preserveScroll: true,
