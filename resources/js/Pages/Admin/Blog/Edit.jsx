...
export default function Edit({ article, categories = [] }) {
    ...
    const [imagePreview, setImagePreview] = useState(article.image ? resolveImagePath(article.image) : null);
    ...
    const handleSubmit = (e) => {
        ...
        Object.keys(formData).forEach(key => {
            ...
            formDataToSend.append(key, value === null ? '' : (typeof value === 'boolean' ? (value ? '1' : '0') : value));
        });
        if (formData.image) {
        ...
        router.post(`/admin/blog/${article.id}`, formDataToSend, {
            forceFormData: true,
            onSuccess: () => {
                setLoading(false);
            },
                    ...
                    <Section title="Informations de Base">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <FormField
                                ...
                                onChange={handleInputChange}
                                options={[{ label: 'Sélectionner...', value: '' }, ...categories.map(cat => ({ label: cat, value: cat }))]}
                            />
                            <FormField
                                label="Auteur"
                                name="author"
                        ...
                        <div className="space-y-4">
                            {imagePreview ? (
                                <div className="relative">
                                    <img src={imagePreview} alt="Preview" className="w-full h-64 object-cover rounded-lg border border-gray-200" />
                                    <button
                                        type="button"
                                        onClick={removeImage}
