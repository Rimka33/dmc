...
export default function Edit({ product, categories = [] }) {
    ...
    const initialTags = product.tags ? (typeof product.tags === 'string' ? JSON.parse(product.tags) : product.tags) : [];
    ...
    useEffect(() => {
        setData('new_images', imagePreview.map(img => img.file));
                        ...
                        <Section title="Gestion des Images & Vidéos">
                            <div className="space-y-6">
                                ...
                                <label className="flex items-center gap-3 px-6 py-8 border-2 border-dashed border-forest-green/20 bg-forest-green/5 rounded-2xl cursor-pointer hover:border-forest-green transition-all group">
                                    ...
                                    <div>
                                        <p className="font-bold text-dark-green uppercase text-xs tracking-widest">Ajouter des médias</p>
                                        <p className="text-[10px] text-dark-green/40 font-bold uppercase tracking-tighter mt-1">Images ou vidéos (Max 50MB)</p>
                                    </div>
                                    <input
                                        type="file"
                                        multiple
                                ...
                                {existingImages.length > 0 && (
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between ml-1">
                                            <p className="text-[10px] font-black text-dark-green/40 uppercase tracking-[0.2em]">Images Actuelles</p>
                                            <p className="text-[9px] font-bold text-forest-green uppercase italic">* Cliquez sur l'étoile pour définir l'image principale</p>
                                        </div>
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            {existingImages.map((media) => (
                                                <div key={media.id} className={`relative group rounded-xl overflow-hidden border-2 aspect-square transition-all ${media.is_primary ? 'border-neon-green shadow-lg' : 'border-forest-green/10'}`}>
                                                    <img
                                ...
                                {imagePreview.length > 0 && (
                                    <div className="space-y-3">
                                        <p className="text-[10px] font-black text-forest-green uppercase tracking-[0.2em] ml-1">Nouveaux Médias</p>
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            {imagePreview.map((img) => (
                                                <div key={img.id} className={`relative group rounded-xl overflow-hidden border-2 aspect-square transition-all ${img.is_primary ? 'border-neon-green shadow-lg' : 'border-forest-green/30'}`}>
                                                    {img.type === 'video' ? (
