...
export default function Newsletter({ subscriptions = {}, stats = {}, filters = {} }) {
    ...
    const columns = [
        ...
        },
        {
            ...
            width: '20%',
            render: (value) => <span className="text-gray-600 text-sm">{value ? new Date(value).toLocaleDateString('fr-FR') : '-'}</span>
        },
        {
            key: 'is_active',
            label: 'Statut',
            ...
            render: (value, row) => (
                <button
                    onClick={() => {
                        router.post(`/admin/newsletter/${row.id}/toggle-status`, {}, {
                            preserveScroll: true,
                            onSuccess: () => router.reload()
                        });
                    }}
                    className={`px-3 py-1 rounded-full text-xs font-bold transition-colors ${
                        value ? 'bg-green-50 text-green-600 hover:bg-green-100' : 'bg-red-50 text-red-600 hover:bg-red-100'
                    }`}
                >
                    {value ? '✓ Actif' : '✗ Inactif'}
                </button>
