        ...
        <AdminLayout>
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {roles.map(role => (
                        <div key={role.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col h-full hover:shadow-md transition-all group relative overflow-hidden">
                            {role.slug === 'admin' && (
                            ...
                            <div className="flex items-start justify-between mb-4">
                                <div className={`p-3 rounded-xl ${role.slug === 'admin' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'}`}>
                                    <Shield size={24} />
                                </div>
                            ...
                            <div className="flex flex-wrap gap-2 mb-6">
                                {role.permissions && role.permissions.slice(0, 3).map(perm => (
                                    <span key={perm.id} className="px-2 py-1 bg-gray-50 text-gray-500 text-[10px] uppercase font-bold rounded border border-gray-100">
                                        {perm.name}
                                    </span>
                                ))}
