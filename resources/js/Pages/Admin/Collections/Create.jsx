...
export default function Create({ products = [], types = [], defaultType = 'custom' }) {
    const getDefaultName = (type) => {
        ...
        return names[type] || 'Collection personnalisée';
    };

    const { data, setData, post, processing, errors } = useForm({
        name: getDefaultName(defaultType),
        slug: '',
        description: '',
        type: defaultType,
        sort_order: 0,
    ...
    // Mettre à jour le nom quand le type change
    useEffect(() => {
        if (data.type && !data.name) {
            setData('name', getDefaultName(data.type));
        }
    }, [data.type]);
    ...
    const handleAddProduct = (product) => {
        if (!selectedProducts.find(p => p.id === product.id)) {
            ...
            setData('products', updated.map(p => ({ id: p.id, sort_order: p.sort_order })));
        }
    };
        ...
        <AdminLayout>
            <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <Link href="/admin/collections" className="flex items-center gap-1 text-gray-500 hover:text-forest-green mb-2 text-sm">
                            <ArrowLeft size={16} /> Retour à la liste
                        </Link>
                        <h1 className="text-2xl font-bold text-gray-900">Nouvelle Collection</h1>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-sm font-semibold text-gray-700">Nom de la collection *</label>
                                <input
                                    type="text"
                                    value={data.name}
                            ...
                            <div className="space-y-2">
                                ...
                                {errors.type && <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle size={12} /> {errors.type}</p>}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                />
                                <p className="text-[10px] text-gray-400 italic">Laissé vide, le slug sera généré automatiquement.</p>
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    max="50"
                                />
                                <p className="text-[10px] text-gray-400 italic">Nombre maximum de produits à afficher</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="space-y-2 max-h-96 overflow-y-auto border border-gray-200 rounded-lg p-2">
                                    {selectedProducts
                                        ...
                                        .map((product, index) => (
                                        <div key={product.id} className="flex items-center gap-3 p-3 bg-white border border-gray-200 rounded-lg hover:border-forest-green transition-all group">
                                            <GripVertical className="text-gray-400 group-hover:text-forest-green cursor-move" size={18} />
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {product.price && (
                                                    <p className="text-xs text-gray-600 font-medium">{parseFloat(product.price).toLocaleString('fr-FR')} FCFA</p>
                                                )}
                                            </div>
                        ...
                        {selectedProducts.length === 0 && (
                            <div className="text-center py-8 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg">
                                ...
                                <p className="text-xs mt-1">Utilisez la recherche ci-dessous pour ajouter des produits</p>
                            </div>
                        )}
                            ...
                            <div className="max-h-96 overflow-y-auto border border-gray-200 rounded-lg mt-2">
                                {filteredProducts.length > 0 ? (
                                    filteredProducts.map(product => {
                                        const productImage = product.images?.[0]?.image_path || product.primary_image || null;
                                        return (
                                            <div
                                                key={product.id}
                    ...
                    <div className="flex justify-end gap-4">
                        ...
                        >
                            <Save size={20} />
                            {processing ? 'Enregistrement...' : 'Enregistrer la collection'}
                        </button>
                    </div>
                </form>
            </div>
