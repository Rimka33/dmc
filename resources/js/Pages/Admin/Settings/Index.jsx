...
import { Settings, Lock, Globe, Bell, Save, CheckCircle, DollarSign, ShieldCheck, Mail } from 'lucide-react';
...
export default function SettingsIndex({ settings = {}, roles = [], flash = {} }) {
    ...
    const { data: paymentData, setData: setPaymentData, post: postPayment, processing: paymentProcessing } = useForm({
        ...
        payment_bank_transfer: settings.payment_bank_transfer === '1' || settings.payment_bank_transfer === true,
        payment_cod: settings.payment_cod === '1' || settings.payment_cod === true,
        stripe_key: settings.stripe_key || '',
        paypal_client_id: settings.paypal_client_id || '',
    });

    const { data: notifData, setData: setNotifData, post: postNotif, processing: notifProcessing } = useForm({
        group: 'notifications',
        ...
        notify_payment_failed: settings.notify_payment_failed === '1' || settings.notify_payment_failed === true,
        notify_customer_review: settings.notify_customer_review === '1' || settings.notify_customer_review === true,
        notify_customer_message: settings.notify_customer_message === '1' || settings.notify_customer_message === true,
        ...
        <AdminLayout>
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 px-6 py-5 font-black text-[10px] uppercase tracking-[0.2em] border-b-2 transition-all whitespace-nowrap ${activeTab === tab.key
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        <form onSubmit={submitGeneral} className="space-y-6">
                            ...
                            <Section title="Réseaux Sociaux" icon={Globe}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <FormField label="Facebook" value={generalData.facebook_url} onChange={e => setGeneralData('facebook_url', e.target.value)} placeholder="https://..." />
                                    <FormField label="Instagram" value={generalData.instagram_url} onChange={e => setGeneralData('instagram_url', e.target.value)} placeholder="https://..." />
                                    <FormField label="Twitter (X)" value={generalData.twitter_url} onChange={e => setGeneralData('twitter_url', e.target.value)} placeholder="https://..." />
                                    <FormField label="LinkedIn" value={generalData.linkedin_url} onChange={e => setGeneralData('linkedin_url', e.target.value)} placeholder="https://..." />
                                </div>
                            </Section>

                            <div className="flex justify-end p-6 bg-white rounded-3xl shadow-sm">
                                <button disabled={generalProcessing} className="flex items-center gap-2 px-8 py-4 bg-forest-green text-white rounded-xl hover:bg-dark-green transition-all font-black text-[10px] uppercase tracking-widest shadow-lg shadow-forest-green/20">
                                    <Save size={16} /> {generalProcessing ? 'Enregistrement...' : 'Sauvegarder les réglages'}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                {activeTab === 'payment' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        <form onSubmit={submitPayment} className="space-y-6">
                            <Section title="Méthodes de Paiement" icon={DollarSign}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {[
                                        ...
                                        <label key={method.key} className="flex items-center justify-between p-5 border border-gray-100 rounded-2xl hover:bg-gray-50 cursor-pointer transition-all">
                            ...
                            <div className="flex justify-end p-6 bg-white rounded-3xl shadow-sm">
                                <button disabled={paymentProcessing} className="flex items-center gap-2 px-8 py-4 bg-forest-green text-white rounded-xl hover:bg-dark-green transition-all font-black text-[10px] uppercase tracking-widest shadow-lg shadow-forest-green/20">
                                    <Save size={16} /> {paymentProcessing ? 'Enregistrement...' : 'Mettre à jour les paiements'}
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {roles.map((role) => (
                                <Section key={role.id} title={role.name} icon={ShieldCheck}>
                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-6 border-b border-gray-50 pb-2">Autorisations actuelles</p>
                                    <div className="space-y-3">
                                        {[
                                            ...
                                            <div key={perm.key} className="flex items-center justify-between p-3.5 bg-gray-50 rounded-xl">
                                                <span className="text-xs font-bold text-gray-600 uppercase tracking-tighter">{perm.label}</span>
                                                <div className={`w-9 h-4.5 rounded-full relative transition-colors ${role.permissions?.[perm.key] ? 'bg-forest-green shadow-lg shadow-forest-green/20' : 'bg-gray-200'}`}>
                                                    <div className={`absolute top-0.5 w-3.5 h-3.5 bg-white rounded-full transition-all ${role.permissions?.[perm.key] ? 'right-0.5' : 'left-0.5'}`}></div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                ...
                {activeTab === 'notifications' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                        <form onSubmit={submitNotif} className="space-y-6">
                            <Section title="Préférences des Alertes" icon={Bell}>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {[
                                        ...
                                        <label key={notif.key} className="flex items-center justify-between p-5 border border-gray-100 rounded-2xl hover:bg-gray-50 cursor-pointer transition-all">
                            ...
                            <div className="flex justify-end p-6 bg-white rounded-3xl shadow-sm">
                                <button disabled={notifProcessing} className="flex items-center gap-2 px-8 py-4 bg-forest-green text-white rounded-xl hover:bg-dark-green transition-all font-black text-[10px] uppercase tracking-widest shadow-lg shadow-forest-green/20">
                                    <Save size={16} /> {notifProcessing ? 'Enregistrement...' : 'Sauvegarder les alertes'}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            {flash.success && (
                <div className="fixed bottom-10 right-10 bg-black text-white px-8 py-5 rounded-3xl shadow-2xl border border-neon-green/30 flex items-center gap-4 animate-in fade-in slide-in-from-right-10 duration-500 z-50">
                    ...
                    <div>
                        <p className="font-black text-xs uppercase tracking-[0.1em]">Configuration mise à jour</p>
