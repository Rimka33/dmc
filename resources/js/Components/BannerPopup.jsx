...
export default function BannerPopup() {
  ...
  const imageUrl =
    window.innerWidth < 768 && banner.mobile_image ? banner.mobile_image : banner.image;
  ...
  return (
    ...
    >
      ...
      {/* Popup Content */}
      <div
        className={`relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden transform transition-all duration-300 ${isClosing ? 'scale-95' : 'scale-100'}`}
      >
        {/* Close Button */}
        <button
          onClick={handleClose}
